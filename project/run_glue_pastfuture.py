...
from pastfuture.configuration_bert import BertConfig
from pastfuture.configuration_albert import AlbertConfig
...
def train(args, train_dataset, model, tokenizer, freeze_lower=False):
    """ Train the model """
...
# Loop to handle MNLI double evaluation (matched, mis-matched)
    ...
    eval_outputs_dirs = (args.output_dir+"_"+str(args.patience), args.output_dir+"_"+str(args.patience) + "-MM") if args.task_name == "mnli" else (args.output_dir,)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if args.eval_flops:
            result["flops"] = total_flops / preds.shape[0]
        results.update(result)

        strng = "eval_L"+"_p"+str(args.patience)+"_results.txt"
...
def main():
    ...
    parser.add_argument("--max_q_len", type=int, default=3)
    parser.add_argument("--set_q", action='store_true')
    parser.add_argument("--freezed_lower_layer", type=int, default=0)
    parser.add_argument("--distll_interval", type=int, default=5)
    ...
    # single_output_layer_param_num = sum(param.numel() for param in model.classifiers.parameters())
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        patience_list = [eval(x) for x in args.patience.split(',')]
