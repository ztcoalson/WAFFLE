...
class DeebertModelWrapper(PyTorchModelWrapper):

    def __init__(self, model, tokenizer, max_seq_length, multiexit, attgoal):
        assert isinstance(
    ...
    def __call__(self, text_input_list):
        ...
        exit_logits = None
        if (self.multiexit):
            exit_logits = outputs[2]
    
        data = [OutputData(int(exit_layer[0]), exit_logits)]
    ...
    def get_grad(self, text_input, loss_fn=CrossEntropyLoss(), adv_train=False):
        ...
        # : a single logit, 12 (logits + others), hidden_states, attentions, highway_exits
        final_logit, total_logits, _, _, _ = self.model(**inputs_dict)

        # only taking the logit values from the outputs
        total_logits = [each_logit[0] for each_logit in total_logits]
