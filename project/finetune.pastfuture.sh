...
for tidx in "${!TASKS[@]}"; do
    ...
    RUN_OUTS=$HOME_DIR/model_output/$BASETYPE"_pastfuture_"$TASKNAME"_"${LEARN[$tidx]}"_"${BATCH[$tidx]}/
    ...
    # : fine-tune
    echo "[finetune.pastfuture.sh] Fine-tune the model"
    if [ "$BASETYPE" = "roberta" ]; then
        python -u run_glue_pastfuture.py \
            ...
            --save_steps 1000 \
            --overwrite_cache \
            --gradient_accumulation_steps ${GRADS[$tidx]} \
            --freezed_lower_layer ${FREEZ[$tidx]} \
