...
class DeeBertEncoder(nn.Module):
    ...
    def forward(
        ...
        # Wasn't working with A2T
        # if self.multiexit and self.training:
        #     raise ValueError("should not be training and attacking at same time")
        all_hidden_states = ()
        all_attentions = ()
        all_highway_exits = ()
        exited = False
        ...
        if self.multiexit and exited:
            new_output = new_output + (all_highway_exits,)
            raise HighwayException(new_output, exit_layer + 1)

        #did not exit early, note that last layer is processed differently to be similar to original BERT
        #this is different than early exiting also at the last layer?
...
class DeeBertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(
        ...
        if not self.training:
            for highway_exit in all_exits:
                highway_logits = highway_exit[0]
                highway_logits_all.append(highway_logits)
            if exit_layer == self.config.num_hidden_layers and not exception:
                #since processes logits different than early exit
                highway_logits_all[self.config.num_hidden_layers-1] = logits
                
            outputs = outputs + (highway_logits_all,) + ((original_entropy, highway_entropy), exit_layer)
            if output_layer >= 0:
                outputs = (
