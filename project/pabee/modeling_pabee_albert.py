...
class AlbertModelWithPabee(AlbertModel):
    def __init__(self, config):
        ...
        self.terminal_inf_layer = -1
        self.total_num_layers = -1
        self.regression_threshold = 0
        self.multiexit = False

    def get_layer_info(self):
        return self.total_num_layers, self.terminal_inf_layer
    ...
    def forward(
        ...
        if self.training:
            ...
            for i in range(self.config.num_hidden_layers):
                ...
                res.append(logits)
        elif self.patience == 0 and not self.multiexit:  # Use all layers for inference
            ...
            self.terminal_inf_layer = self.total_num_layers

        else:
            if self.patience == 0:
                self.patience = self.config.num_hidden_layers
            ...
            for i in range(self.config.num_hidden_layers):
                ...
                #still need to save logits from all exits if multiexit
                if patient_counter == self.patience and not exited:
                    exit_layer = calculated_layer_num
                    exited = True
            ...
            self.inference_layers_num += exit_layer
            self.inference_instances_num += 1
            self.terminal_inf_layer = [[exit_layer] for x in range(len(patient_result))]

        return res, torch.tensor([[self.total_num_layers]],dtype=torch.int32,device=device).detach(), torch.tensor(self.terminal_inf_layer, dtype=torch.int32,device=device).detach(), all_logits
...
class AlbertForSequenceClassificationWithPabee(AlbertPreTrainedModel):
    ...
    def forward(
        ...
        """
        logits, total_num_layers, terminal_inf_layer, all_logits = self.albert(
            input_ids=input_ids,
        ...
        return outputs, total_num_layers, terminal_inf_layer, all_logits
