...
from utils import _get_data_runglue
...
def evaluate(args, model, tokenizer, prefix="", patience=0):
    ...
    eval_outputs_dirs = (args.output_dir+"_"+str(patience), args.output_dir+"_"+str(patience) + "-MM") if args.task_name == "mnli" else (args.output_dir,)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if args.output_mode == "classification":
            ...
            #maybe create a new regression_threshold arg instead of reusing the one for determining patience
            matches = np.where(((preds >= (out_label_ids-args.regression_threshold)) & (preds <= (out_label_ids+args.regression_threshold))), True,False).astype(int)
        result = compute_metrics(eval_task, preds, out_label_ids)
        results.update(result)
        
        strng = "matches_p"+str(patience)+".txt"
        fpath =  os.path.join(eval_output_dir, prefix, strng)
        np.savetxt(fpath, matches, fmt='%i')

        strng = "exit_layer_p"+str(patience)+".txt"
        fpath =  os.path.join(eval_output_dir, prefix, strng)
        np.savetxt(fpath, terminal_inf_layer, fmt='%i')

        strng = "idx_p"+str(patience)+".txt"
        fpath =  os.path.join(eval_output_dir, prefix, strng)
        np.savetxt(fpath, idx, fmt='%i')

        # print("total num layers = "+str(total_num_layers))

        strng = "eval_L"+str(total_num_layers)+"_p"+str(patience)+"_results.txt"
        output_eval_file = os.path.join(eval_output_dir, prefix, strng)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if task.lower() in ["mnli", "mnli-mm"] and args.model_type in ["roberta", "xlmroberta"]:
            # HACK(label indices are swapped in RoBERTa pretrained model)
            label_list[1], label_list[2] = label_list[2], label_list[1]

        examples =_get_data_runglue(task, args, evaluate, processor)
    ...
    idx = torch.tensor([i for i in range(len(features))], dtype=torch.long)

    dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels, idx)
    return dataset
