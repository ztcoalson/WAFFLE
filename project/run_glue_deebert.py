...
def evaluate(args, model, tokenizer, prefix="", output_layer=-1, eval_highway=False):
    ...
    eval_outputs_dirs = (args.output_dir+"_"+str(args.early_exit_entropy), args.output_dir+"_"+str(args.early_exit_entropy) + "-MM") if args.task_name == "mnli" else (args.output_dir,)
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        logger.info("[Mine][{}] Eval time: {}".format(args.task_name, eval_time))
        ...
        if eval_highway:
            logger.info("[Mine][{}] Exit layer counter: {}".format(args.task_name, exit_layer_counter))
            actual_cost = sum([l * c for l, c in exit_layer_counter.items()])
            full_cost = len(eval_dataloader) * model.num_layers
            logger.info("[Mine][{}] Expected saving  : {}".format(args.task_name, actual_cost / full_cost))
            logger.info("[Mine][{}] Expected speed-up: {}".format(args.task_name, full_cost / actual_cost))
            if args.early_exit_entropy >= 0:
                ...
                logger.info("[Mine][{}] Entropy={}\tResult={:.2f}".format( \
                    args.task_name, args.early_exit_entropy, 100 * print_result))

        strng = "eval_L"+"_p"+str(args.early_exit_entropy)+"_results.txt"
        output_eval_file = os.path.join(eval_output_dir, prefix, strng)
        with open(output_eval_file, "w") as writer:
            logger.info("[Mine][{}] ***** Eval results {} *****".format(args.task_name, prefix))
            for key in sorted(result.keys()):
                logger.info("[Mine][%s] %s = %s", args.task_name, key, str(result[key]))
                writer.write("%s = %s\n" % (key, str(result[key])))
