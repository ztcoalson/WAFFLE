...
# ----------------------------------------
#   PabeeModelWrapper class
# ----------------------------------------
class PabeeModelWrapper(PyTorchModelWrapper):
    ...
    def __call__(self, text_input_list):
        ...
        with torch.no_grad():
            outputs, total_num_layers, terminal_inf_layer, all_logits = self.model(**inputs_dict)
        ...
        data = [OutputData(int(x), exit_logits) for x in terminal_inf_layer]
    ...
    def get_grad(self, text_input, loss_fn=CrossEntropyLoss(), adv_train=False):
        ...
        # : a single logit, -1, 11 (total - 1), and 12 logits
        final_logit, num_final_layer, num_early_exits, total_logits = self.model(**inputs_dict)
