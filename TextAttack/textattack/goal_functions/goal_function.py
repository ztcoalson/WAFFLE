...
class GoalFunction(ReprMixin, ABC):
    ...
    def get_results(self, attacked_text_list, check_skip=False):
        ...
        model_outputs, outputs_data = self._call_model(attacked_text_list)
        for attacked_text, raw_output, data in zip(attacked_text_list, model_outputs, outputs_data):
            displayed_output = self._get_displayed_output(raw_output)
            goal_status = self._get_goal_status(
                raw_output, data, attacked_text, check_skip=check_skip
            )
            goal_function_score = self._get_score(raw_output, data, attacked_text)
            results.append(
    ...
    def _get_goal_status(self, model_output, data, attacked_text, check_skip=False):
        should_skip = check_skip and self._should_skip(model_output, data, attacked_text)
        if should_skip:
            return GoalFunctionResultStatus.SKIPPED
        if self.maximizable:
            return GoalFunctionResultStatus.MAXIMIZING
        if self._is_goal_complete(model_output, data, attacked_text):
    ...
    def _should_skip(self, model_output, data, attacked_text):
        return self._is_goal_complete(model_output, data, attacked_text)
    ...
    def _call_model_uncached(self, attacked_text_list):
        ...
        while i < len(inputs):
            ...
            batch_preds, data = self.model(batch)
        ...
        if self.multiexit_data:
            res = self._process_model_outputs(attacked_text_list, outputs, outputs_data)
        else:
            res = self._process_model_outputs(attacked_text_list, outputs)

        assert len(res) == len(
            outputs_data
        ), f"Got data {len(outputs_data)} outputs for {len(res)} inputs"

        return res, outputs_data
    ...
    def _call_model(self, attacked_text_list):
        ...
        if not self.use_cache:
            outputs, outputs_data = self._call_model_uncached(attacked_text_list)
            ...
            outputs, outputs_data = self._call_model_uncached(uncached_list)
            for text, output, out_data in zip(uncached_list, outputs, outputs_data):
                self._call_model_cache[text] = (output, out_data)
            all_outputs = [self._call_model_cache[text] for text in attacked_text_list]
            outputs, outputs_data = zip(*all_outputs)
            return outputs, outputs_data 
