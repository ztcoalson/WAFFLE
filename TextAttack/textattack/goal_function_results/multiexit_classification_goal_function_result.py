...
#todo, everything, make sure doesn't break, see what results want to output
...
# --------------------------------------------------------------------------------
#   MultiexitClassificationGoalFunctionResult class
# --------------------------------------------------------------------------------
class MultiexitClassificationGoalFunctionResult(GoalFunctionResult):
    ...
    def _processed_output(self):
        ...
        output_label = self.raw_output[self.data.layer-1].argmax()
        if self.attacked_text.attack_attrs.get("label_names"):
            output = self.attacked_text.attack_attrs["label_names"][self.output[self.data.layer-1]]
    ...
    def get_colored_output(self, color_method=None):
        ...
        #import ipdb; ipdb.set_trace()
        output_label = self.raw_output[self.data.layer-1].argmax()
        confidence_score = self.raw_output[self.data.layer-1][output_label]
