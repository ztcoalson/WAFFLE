...
# for the slowdown attack
from textattack.metrics.attack_metrics import (
    SlowdownSuccessRate,
    SlowdownWordsPerturbed,
)
...
class AttackLogManager:
    ...
    def log_result(self, result, slowdown=False):
        """Logs an ``AttackResult`` on each of `self.loggers`."""
        self.results.append(result)
        for logger in self.loggers:
            logger.log_attack_result(result, slowdown=slowdown)

    def log_results(self, results, slowdown=False):
        """Logs an iterable of ``AttackResult`` objects on each of
        `self.loggers`."""
        for result in results:
            self.log_result(result, slowdown=slowdown)
        self.log_summary(slowdown=slowdown)
    ...
    def log_summary(self, slowdown=False):
        total_attacks = len(self.results)
        if total_attacks == 0:
            return

        # --------------------------------------------------------------------------------
        #   Use the standard metrics
        # --------------------------------------------------------------------------------
        if not slowdown: 
        ...
        # --------------------------------------------------------------------------------
        #   Use the slowdown metrics
        # --------------------------------------------------------------------------------
        else:
            attack_success_stats = SlowdownSuccessRate().calculate(self.results)
            words_perturbed_stats = SlowdownWordsPerturbed().calculate(self.results)
            attack_query_stats = AttackQueries().calculate(self.results)
            ...
            summary_table_rows = [
                ...
                # : slowdown accuracy computed from the total.
                [
                    "Original accuracy:",
                    str(attack_success_stats["original_accuracy"]) + "%",
                ],
            ...
            summary_table_rows.append(
                [
                    "Avg num queries:", 
                    attack_query_stats["avg_num_queries"]
                ]
            )
